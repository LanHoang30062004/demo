...
public class UserController {   
    ...
    public ResponseData<?> deleteUser(@PathVariable(name = "id") Long userId) {
        try {
            ...
            return new ResponseData<>(HttpStatus.OK.value(), "Delete user successful with id : " + userId);
        }
        catch (Exception e) {
    ...
    public ResponseData<?> getProductInCart(@PathVariable(name = "productId") Long productId) {
        try {
            Boolean result = this.userService.existsByProduct(productId) ;
            log.info("Get products in cart successful with productId : {}", productId);
            return new ResponseData<>(HttpStatus.OK.value(),"Get products in cart successful with productId :" + productId , result);
        }
        catch (Exception e) {
            log.error("Can not get products in cart : {}", e.getMessage());
            return new ResponseData<>(HttpStatus.INTERNAL_SERVER_ERROR.value() ,"Can not get products in cart with product id : " + productId);
        }
    }
