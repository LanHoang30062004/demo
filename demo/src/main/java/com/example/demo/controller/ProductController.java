...
public class ProductController {
    ...
    @PostMapping("buy/{user}/{product}")
    public ResponseData<?> buyProductToCart(@PathVariable("user") String user, @PathVariable("product") Long product) {
        try {
            this.productService.buyProductToCart(user, product);
            log.info("Buy product to cart successful with user id :" + user);
            return new ResponseData<>(HttpStatus.OK.value(), "Buy product to cart successful with user id : " + user);
        } catch (Exception e) {
            log.error("Buy product to cart failed : {}", e.getMessage());
            return new ResponseData<>(HttpStatus.OK.value(), "Buy product to cart failed");
        }
    }
